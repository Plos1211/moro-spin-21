...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-4b23d7da";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-4b23d7da">'+'<span class="tile-face-4b23d7da tile-front-4b23d7da"></span>'+'<span class="tile-face-4b23d7da tile-back-4b23d7da"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-4b23d7da")){return;}
var back=tile.querySelector(".tile-back-4b23d7da");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-4b23d7da");if(isMine){tile.classList.add("boom-4b23d7da");}
if(dim){tile.classList.add("dimmed-4b23d7da");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-4b23d7da")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-4b23d7da");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-4b23d7da")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-4b23d7da");void panelEl.offsetWidth;panelEl.classList.add("jolt-4b23d7da");}
endRound();}
